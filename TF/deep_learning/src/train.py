...
SAVE_DIR = "/".join([WORKING_PATH, "data", "saved_images"])
WRITER_PATH = "/".join([WORKING_PATH, "deep_learning", "src", "board_vgg_aspp"])
CHECKPOINT_PATH = "/".join([WORKING_PATH, "deep_learning", "src", "checkpoints", "unet_vgg_aspp.pth"])
TRAIN_DIR = "/".join([WORKING_PATH, "data", "train"])
VAL_DIR = "/".join([WORKING_PATH, "data", "val"])
TEST_DIR = "/".join([WORKING_PATH, "data", "test"])
...
if __name__ == "__main__":
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch > 20 and BEST_VAL_IOU < val_metrics['mean_iou']:
            BEST_EPOCH = epoch
